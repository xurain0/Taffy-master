...
class test_Uao(object):
    ...
    def getResult(self, inseqno, respseqno, type):
        ...
        inMaxNo = self.getValue(findinMaxNosql)[0]
        #processingnosql = 'select t.processingno from t_ineSeqProcess t  where t.sender = \'cfmmc\'  and t.seqno > '+inseqno
        clientregionsql = 'select t.clientregion,t.processingno from t_ineSeqProcess t  where t.sender = \'cfmmc\'  and t.seqno > '+inseqno
        #processingno = self.getValue(processingnosql)
        #clientregion = self.getValue(clientregionsql)
        ...
        for c in clientregion:
            ...
            #clientregionNo = 2,3 境外客户
            if clientregionNo == 2 or clientregionNo == 3:
                count = count + 1
                while True:
                    #查询t_operationlog，直到processingno这条记录处理完，去页面处理
                    # processingnosql = 'select t.processingno from t_ineSeqProcess t  where t.sender = \'cfmmc\'  and t.seqno = '+seqNo
                    # processingno = self.getValue(processingnosql)
                    #operationsql = 'select * from party.t_operationlog t where t.logdesc like \'%ProcessingNo: '+processingno[0]+'%\''
                ...
                if num > 0:
                    for n in range(1, num+1):
                        while True:
                            ...
                            maxRseqno = self.getValue(rSeqnosql)[0]
                            #genMaxNo = self.getValue(genMaxNosql)[0]
        ...
        if count > 0:
            ...
            for p in processingnolist:
                analysisRessql1 = 'select  t.seqno,t.exreturncode,t.exreturnmsg  from t_ineSeqProcess t  where t.sender = \'N\' and t.processingno ='+p+' order by t.seqno desc'
                analysisRes1 = self.orcl.connOrcl(analysisRessql1)[2]
                for i in analysisRes1:
                    if i[1] <> 0:
                        print 'seqno = '+str(i[0])+' operate filed and errmsg: ' + str(i[2])
                    else:
                        print 'seqno = '+str(i[0])+' operate success and sucmsg: ' + str(i[2])
            ...
            #         break
            #     for p in processingnolist:
            #         analysisRes = 'select  t.seqno,t.exreturncode,t.exreturnmsg  from t_ineSeqProcess t  where t.sender = \'N\' and t.processingno ='+p+' order by t.seqno desc'
            #
            #     genMaxNo = self.getValue(genMaxNosql)[0]
